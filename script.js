...
function renderCatalog() {
        ...
        <div class="ppanel ${activeClass}" id="panel_${pkg.id}">
          ...
          <div>
            <div class="price-strip">
              ...
              <span class="p-val">${pkg.price}</span>
              ${pkg.priceNote ? `<span class="p-note">${pkg.priceNote}</span>` : ''}
            </div>
            <button class="svc-card-btn" onclick="openEnquiryModal('${pkg.serviceName} - ${pkg.packageName}', '${pkg.price} ${pkg.priceNote || ''}')">
  ...
  // Re-observe reveal elements for scroll animation
...
let currentEnquiryPrice = '';

window.openEnquiryModal = function (productName, price) {
  currentEnquiryProduct = productName;
  currentEnquiryPrice = price;

  if (modalTitle) modalTitle.textContent = productName;
  if (modalPrice) modalPrice.textContent = price;

  if (messageInput) {
    messageInput.value = `Hi, I am interested in your "${productName}" service listed at ${price}. Please share more details and steps to get started.`;
  }
...
// Handle submission redirecting to WhatsApp api
...
Product Name: ${currentEnquiryProduct}
Price: ${currentEnquiryPrice}
...
function initSearchAndFilters() {
  ...
  if (filterContainer) {
    // Generate unique category list from products + 'All'
